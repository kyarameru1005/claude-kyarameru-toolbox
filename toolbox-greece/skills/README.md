...
- `muse-playful`: デザイン系統（ポップ/楽しい）。明るい多色・丸み・弾むモーションのトーン。
- `muse-cute`: デザイン系統（可愛い/kawaii）。パステル・やわらかい丸み・控えめな動きのトーン。
- `muse-editorial`: デザイン系統（雑誌/エディトリアル）。セリフ・大きな見出し・非対称グリッドのトーン。
- `muse-retro`: デザイン系統（レトロ/ノスタルジック）。くすんだ暖色・ヴィンテージ書体・粗い質感のトーン。
- `muse-tech`: デザイン系統（テック/サイバー）。暗背景・ネオン・等幅書体・グリッド線のトーン。
...
- 系統（見た目のトーン）: `muse-modern` / `muse-luxury` / `muse-minimal` / `muse-bold` / `muse-avantgarde` / `muse-playful` / `muse-cute` / `muse-editorial` / `muse-retro` / `muse-tech`
