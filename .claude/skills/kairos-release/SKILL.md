...
7. `CHANGELOG.md` に新 version の見出しと変更点（追加・変更・修正を簡潔に）を追記する。
   利用者は version が上がったことだけでなく **何が変わったか** を知る必要がある。
8. コミットする（`argo-git-flow` を使うと安全）。例: `plugins: バージョンを <new> へ更新`。
9. リリースを固定したい場合は **注釈付きタグ** を打つ。例:
   ```bash
   git tag -a v<new> -m "v<new>"
   git push origin v<new>
   ```
   タグ作成は破壊的でないが、push は外向き操作なので確認の上で行う。タグの強制移動はしない。
10. main へマージ後、利用者は `/plugin marketplace update kyarameru-claude` → `/reload-plugins` で新 version を受け取る。
...
- version は上げるが `CHANGELOG.md` を更新しない（内容が利用者に伝わらない）。
