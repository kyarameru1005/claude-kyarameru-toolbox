...
6. コミットする（`argo-git-flow` を使うと安全）。例: `plugins: バージョンを <new> へ更新`。
7. リリースを固定したい場合は **注釈付きタグ** を打つ。例:
   ```bash
   git tag -a v<new> -m "v<new>"
   git push origin v<new>
   ```
   タグ作成は破壊的でないが、push は外向き操作なので確認の上で行う。タグの強制移動はしない。
8. main へマージ後、利用者は `/plugin marketplace update kyarameru-claude` → `/reload-plugins` で新 version を受け取る。
